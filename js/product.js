document.addEventListener('DOMContentLoaded', () => {
    ...
    <section class="section">
      <div class="container grid-12">
        ...
        <div style="grid-column: span 6;">
          ...
          <p id="precio-mostrado" style="font-size: var(--text-lg); margin: var(--space-2) 0;">$${producto.precios['250g']} MXN</p>

          <div class="field">
            <label>Presentación</label>
            <div class="size-selector" role="group" aria-label="Elegir presentación">
              ${Object.keys(producto.precios).map((tam) => `
                <button class="size-option" data-size="${tam}" aria-pressed="${tam === '250g'}">${tam}</button>
              `).join('')}
            </div>
          </div>
  ...
  // Selector de presentación — actualiza el precio mostrado
  contenedor.querySelectorAll('.size-selector .size-option').forEach((boton) => {
    boton.addEventListener('click', () => {
      ...
      const tamano = boton.dataset.size;
      const precio = producto.precios[tamano];
      document.getElementById('precio-mostrado').textContent = `$${precio} MXN`;
    });
  });
});
